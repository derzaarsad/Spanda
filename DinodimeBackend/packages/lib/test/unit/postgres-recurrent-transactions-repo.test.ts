...
describe("unit: postgres recurrent transactions repository", function() {
  ...
  it("renders the save without an id", async function() {
    ...
    expect(result).to.be.a("string");
    expect(result).to.equal(
      "INSERT INTO recurrenttransactions (accountid,transactionids,isexpense,isconfirmed,frequency,counterpartname) VALUES ('995070','{1,2,3}','t','f','Unknown',NULL) RETURNING id"
    );
  });
  ...
  it("renders the save array without id query", async function() {
    ...
    expect(result).to.be.a("string");
    expect(result).to.equal(
      "INSERT INTO recurrenttransactions (accountid,transactionids,isexpense,isconfirmed,frequency,counterpartname) VALUES ('2','{1,2,3}','true','false','Unknown',NULL), ('2','{4,5,6}','false','false','Unknown',NULL) RETURNING id"
    );
  });
